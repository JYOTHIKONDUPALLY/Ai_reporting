...
async function createInvoiceTable(clickhouse, tableName) {
  const createQuery = `
    ...
    COGS UInt64,
    Commission String,
    co_faet_tax Decimal(18, 2) DEFAULT 0.00,
    ...
    while (offset < totalRecords) {
     ...
     WHERE c.invoiceItemId = ?`,
        ...
        data.push({
          ...
          cogs: Cogs,
          co_faet_tax: Number(safeNum(r.pifTax).toFixed(2)),
          commission: Commission,
          guest_pass_discount: guestpassDiscount,
          membership_discount: membershipDiscount,
          package_discount: packageDiscount,
          refund_amount: r.refund_amount < 0 ? 0 : Number(safeNum(r.returnAmount ?? r.totalReturnAmount).toFixed(2)),
